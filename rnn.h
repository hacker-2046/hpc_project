...
inline xt::xarray<double> RNN::backward(const xt::xarray<double>& error, double lr) {
    ...
    xt::xarray<double> current_hidden, last_hidden, current_output_grad, delta_hidden_current;
    for (int i = seq - 1; i >= 0; i--) {
        current_hidden = xt::view(hidden, xt::all(), i + 1, xt::all());
        last_hidden = xt::view(hidden, xt::all(), i, xt::all());
        ...
        //delta_hidden_current = xt::clip(delta_hidden_current, -5.0, 5.0); // Clip gradients to prevent explosion
    ...
    for (int i = 0; i < seq; i++) {
        xt::view(delta_input, xt::all(), i, xt::all()) = 
            linear_input.backward(
                xt::view(input, xt::all(), i, xt::all()),  // current input
                xt::view(delta_hidden, xt::all(), i+1, xt::all()), //current error
                lr 
            ); 
    }
    return delta_input;
